...
const cfgHomeKey = "CT_CONFIG_HOME"
...
// emptyFunc is used as the InitFn to commands which don't have init
// commands, such as list sub commands.
func emptyFunc() {}
...
func (test *CommandTest) Setup(t *testing.T) {
	...
	test.Cmd.ResetFlags()
	test.Cmd.RunE = nil
	test.Cmd.Run = emptyRun
	test.InitFn()

	test.App = &cobra.Command{Use: "c", Run: emptyRun}
	test.App.AddCommand(test.Cmd)
	test.App.SetOutput(Err)
	test.App.SetArgs(test.Args)
}
